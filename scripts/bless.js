  ...
  try {
    ...
    if (proxy) {
      try {
        ...
        console.log(
          `${Colors.Neon}]> ${Colors.Rt} Using proxy for node ${Colors.Teal}${maskedNodeId(nodeId)}${Colors.Rt} from line ${globalNodeIndex + 1}: ${Colors.Rt}IP: ${Colors.Gold}${maskedIp}${Colors.Rt}`
        )
      } catch (ipError) {
        console.error(
          `${Colors.Red}Failed to fetch/mask proxy IP for node ${maskedNodeId(nodeId)}:${Colors.Rt}`
        )
        throw ipError
      }
    } else {
      console.warn(
        `${Colors.Red}No proxy found for node ${maskedNodeId(nodeId)}. Proceeding without a proxy.${Colors.Rt}`
      )
    }
    ...
    console.log(
      `${Colors.Neon}]> ${Colors.Rt} Node : ${Colors.Teal} ${maskedNodeId(nodeId)}${Colors.Rt}`
    )
    ...
    const statusPing =
      postResponse.status === 200 ? 'Pinging Success!' : 'Pinging Failed!'
    console.log(
      `${Colors.Neon}]> ${Colors.Rt} PING : ${pingColor} ${statusPing}${Colors.Rt}`
    )
    ...
    const statusCheck =
      getResponse.status === 200 ? 'Checking Success!' : 'Checking Failed!'
    console.log(
      `${Colors.Neon}]> ${Colors.Rt} Check: ${getColor} ${statusCheck}${Colors.Rt}`
    )
    ...
    try {
      ...
      totalReward =
        responseData.totalReward !== undefined
          ? responseData.totalReward
          : totalReward
      todayReward =
        responseData.todayReward !== undefined
          ? responseData.todayReward
          : todayReward
    } catch (parseError) {
      console.error(
        `${Colors.Red}Error parsing GET response body for node ${maskedNodeId(nodeId)}:${Colors.Rt}`
      )
    }

    console.log(
      `${Colors.Neon}]> ${Colors.Rt} Point: ${Colors.Rt} totalReward: ${Colors.Neon} ${totalReward} ${Colors.Rt} todayReward: ${Colors.Neon} ${todayReward}${Colors.Rt}`
    )
  } catch (error) {
    console.error(
      `${Colors.Red}Error processing node ${maskedNodeId(nodeId)}:${Colors.Rt}`
    )
    throw error
  } finally {
    if (browser) {
...
async function processNodeWithRetries(
  authToken,
  nodeId,
  proxy,
  globalNodeIndex,
  ...
  while (attempt < maxRetries) {
    try {
      ...
      attempt++
      console.error(
        `${Colors.Red}Attempt ${attempt} failed for node ${maskedNodeId(nodeId)}.${Colors.Rt}`
      )
      if (attempt >= maxRetries) {
        console.error(
          `${Colors.Red}Max retries reached for node ${maskedNodeId(nodeId)}. Skipping...${Colors.Rt}`
        )
      } else {
        console.log(
          `${Colors.Neon}]> ${Colors.Rt}Retrying node ${maskedNodeId(nodeId)} (Attempt ${attempt + 1} of ${maxRetries})...`
        )
      }
    }
  }
...
async function main() {
  ...
  for (let accIndex = 0; accIndex < accounts.length; accIndex++) {
    ...
    console.log(
      `${Colors.Gold}Processing account ${accIndex + 1}...${Colors.Rt}`
    )
    ...
    for (let nodeIndex = 0; nodeIndex < 5; nodeIndex++) {
      ...
      if (!nodeId) {
        console.warn(
          `${Colors.Red}No node ID found for account ${accIndex + 1}, node ${nodeIndex + 1}. Skipping.${Colors.Rt}`
        )
        continue
      }
